...
async function generateTechnicalContent(project, template, language = 'en') {
  log(`🤖 AI生成${language === 'zh' ? '中文' : '英文'}技术文档: ${template.name_zh || template.name_en}`);
...
文档要求：
- 文档类型：${language === 'zh' ? (template.name_zh || template.name_en) : (template.name_en || template.name_zh)}
...
- 长度控制在2000-4000字符之间`;
    ...
    const userPrompt = language === 'zh' 
      ? `请为"${project.name}"项目生成详细的${template.name_zh || template.name_en}。`
      : `Please generate a detailed ${template.name_en || template.name_zh} for the "${project.name}" project.`;
...
async function saveToDatabase(project, template, englishContent, chineseContent, mdcEnglish, mdcChinese, stats) {
  log(`💾 保存到数据库: ${project.name} + ${template.name_zh || template.name_en}`);
  ...
  try {
    const saveData = {
      ...
      input_content: JSON.stringify({
        ...
        template_name: template.name_en || template.name_zh,
...
async function executeLargeScaleBatch() {
  ...
  const results = {
    total: 0,
    success: Array.isArray(progress.completedTasks) ? progress.completedTasks.length : 0,
    failed: 0,
    totalTokens: 0,
    errors: []
  };
