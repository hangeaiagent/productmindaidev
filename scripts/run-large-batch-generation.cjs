 ...
 * 
 * 使用方法：
 * node scripts/run-large-batch-generation.cjs --user_id=xxx --languages=zh,en
 */
...
const CONFIG = {
  ...
  batchSize: 1,           // 每批处理的项目数 - 最小化
  templateBatchSize: 1,   // 每批处理的模板数 - 最小化
  maxExecutionTime: 15000, // 单次执行最大时间(毫秒)，留15秒缓冲
  retryAttempts: 3,       // 失败重试次数
  retryDelay: 2000,       // 重试延迟(毫秒) - 进一步减少
