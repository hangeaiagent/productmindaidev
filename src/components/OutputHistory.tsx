...
const OutputHistory: React.FC = () => {
  const { generatedOutputs, refetchHistory, language } = useAppContext();
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden mb-6">
      ...
      >
        <span>{language === 'zh' ? `历史记录 (${historyItems.length})` : `Previous Outputs (${historyItems.length})`}</span>
          ...
          {historyItems.map((item) => (
            <div key={item.id} className="px-6 py-4">
              <div className="flex justify-between items-center">
                <div>
                  <h4 className="text-sm font-medium text-gray-900">{item.promptTitle}</h4>
                  ...
                  {!item.is_active && (
                    <>
                      <button
                        ...
                        title={language === 'zh' ? '设为当前版本' : 'Set as Current Version'}
                      >
              ...
              <div className="mt-2">
                <p className="text-sm text-gray-600">
                  <span className="font-medium">{language === 'zh' ? '输入：' : 'Input:'}</span> {truncateText(item.input)}
                </p>
              </div>
