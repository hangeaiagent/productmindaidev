...
MONITOR_SCRIPT="$SCRIPTS_DIR/system-monitor-enhanced.sh"
MANAGER_SCRIPT="$SCRIPTS_DIR/system-service-manager-enhanced.sh"
...
install_cron() {
    ...
    # 移除旧的cron任务
    (crontab -l 2>/dev/null | grep -v "system-monitor") | crontab -
...
# 显示使用说明
show_usage() {
    log "\n${GREEN}=== ProductMind AI 监控系统 (增强版) ===${NC}"
...
# 主函数
main() {
    log "${GREEN}开始安装 ProductMind AI 监控系统 (增强版)...${NC}"
